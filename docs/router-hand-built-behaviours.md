...
| **The `invoke` verb on its own transports** — an MCP tool call and an A2A task at the addresses the manifest advertises, each landing on the same ladder as `/v1` | N6 **NOT COVERED** (a client resolves other agents' cards; it serves neither surface) | `invoke.py`, `mcp.py`, `a2a.py` | `apr_invoke`, `apr_mcp`, `apr_a2a`, `apr_jsonrpc_handler` | `tests/test_mcp.py`, `tests/test_a2a.py`, `apr_invoke_SUITE`, the corpus' invoke exchanges |
